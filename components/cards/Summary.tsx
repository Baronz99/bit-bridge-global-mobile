  ...
  return (
    <View className="space-y-3 overflow-hidden">
      ...
      <SummaryRow
        ...
        value={moneyFormat(data?.amount) || '₦0.00'}
        commission={moneyFormat(data?.bill_commission)}
      />
      ...
      <View className=" flex-row gap-4">
        {applyCommission && (
          <Text
            ellipsizeMode="tail"
            numberOfLines={2}
            className="font-medium text-alt "
            style={{ textDecorationColor: 'line-through' }}
          >
            {commission}
          </Text>
        )}
        <Text
          ...
          style={{ textDecorationLine: applyCommission ? 'line-through' : 'none' }}
        >
          {value}
        </Text>
      </View>
