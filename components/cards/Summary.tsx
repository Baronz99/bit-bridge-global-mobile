...
const Summary = ({data}: any) => {
  return (
    ...
    <SummaryRow label={`${data?.service_type == "VTU" ? "Phone Number" : "Meter Number"}`} value={data?.meter_number || "N/A"} />
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <View className="flex-row gap-4 justify-between items-center">
      ...
      <Text ellipsizeMode='tail' numberOfLines={2} className="font-medium text-gray-100">{value}</Text>
    </View>
  );
}
