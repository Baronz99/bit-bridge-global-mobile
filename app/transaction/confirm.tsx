...
const confirm = () => {
  const {authState: {token}, userProfileData} = useAuth()
    ...
    // const reference = "bbg-1745947853"
    const {data} = useFetch(()=> getTransactionRecord({
    ...
    console.log("fetched data-===>", receipt_type, data)
  ...
  return (
    <View className='flex-1 px-4 bg-primary'>
      ...
      Failed  transaction
       ...
       {data?.status === "approved" ? <Image source={images.success} className='w-40 m-auto h-40' />
      ...
      <View className='m-auto w-full px bg-r'>
        <View className='my-10'> 
          <Text className='text-white text-center my-0 text-3xl'>{moneyFormat(data?.anount ?? 200)}</Text>
        </View>
          <Text className='text-white text-center my-8 text-2xl'>{(data?.description ?? "Airtime Recharge")}</Text>
          <Text className='text-white text-center my-4 text-3xl'>{(data?.meter_number ?? "07064334160")}</Text>
