...
const OrderDetails = () => {
    ...
    const {data, loading} = useFetch(() => getPurchaseOrder({
        id,
        token
  ...
  return (
    ...
    <Text className="text-2xl font-bold text-white mb-6">Order Summary</Text>
      ...
      <Text className='text-2xl text-center text-alt font-semibold'>  {data?.token} </Text>
        ...
        {data.service_type === "ELECTRICITY" && <DetailLine value={data.address} label={"Address"}/>    }
        <DetailLine value={data?.meter_number} label={ (data.service_type === "DATA" || data.service_type === "VTU" ) ? "Phone Number": "Meter Number"}/>

        <DetailLine value={data?.status} label={"Status"}/>
        {data.service_type === "ELECTRICITY" && <DetailLine value={data.units} label={"Units"}/>}
...
const DetailLine = ({
    ...
    <View className='flex-row justify-between gap-3 items-center py-2'>
        ...
        <Text
            numberOfLines={2} className='text-white font-semibold '>
            {value}
        </Text>
    </View>
)
