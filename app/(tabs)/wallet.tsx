        ...
        <View className="mt-7 border border-gray-800 bg-primary rounded-lg overflow-hidden max-h-96">
          ...
          <ScrollView className="">
            {loading ? (
              ...
              data &&
              (data.data.length < 1 ? (
                <View className="flex-row border-t border-gray-600 px-4 py-4">
                ...
                data?.data.map((item, index) => (
                  <>
                    <View key={index} className="flex-row border-t border-gray-600 px-4 py-2">
                      <Text
                        className={`${item.status === 'approved' ? 'text-green-500' : item.status === 'initialized' ? 'text-gray-200' : 'text-red-600'} flex-1 `}
                      >
                        {item.status}
                      </Text>
