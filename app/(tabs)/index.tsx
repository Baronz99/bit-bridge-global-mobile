          ...
          <View className="flex-1">
            ...
            {userProfileData?.account && (
              <TouchableOpacity
                ...
                className="my-4 bg-gray-900   py-2 w-48 flex flex-row gap-4 items-center rounded-2xl px-4"
              >
            ...
            <View className="flex flex-row gap-3 w-full overflow-x-scroll">
              {services.map((item) => (
                <TouchableOpacity key={item.id} onPress={() => setSelectedService(item.name)}>
                  <Text className="text-white  font-semibold w-max bg-app-primary py-2.5 px-4 rounded-md">
                    {item.label}
                  </Text>
                </TouchableOpacity>
              ))}
        ...
        <AppModal onclose={() => setOpenModal(false)} open={openModal}>
          <View className="f bg-black/70 justify-center items-center px-6">
            <View className="bg-gray-900 p-6 rounded-2xl w-full max-w-md">
              ...
              <View className="">
                <Text
                  className={`${billOrder?.biller === 'MTN' ? 'text-alt' : billOrder?.biller === 'GLO' ? 'text-green-500' : 'text-white'}  font-semibold text-center text-lg`}
                >
                  {billOrder?.biller}
                </Text>
                ...
                <LabelText
                  label={'Description'}
                  value={`subscription ${billOrder?.service_type} `}
                />
                ...
                renderItem={({ item }: any) => (
                  <React.Fragment key={item?.id}>
                    <Link href={`/mobileProviders/${item.id}`} asChild>
                      <TouchableOpacity className="bg-gray-800/50 overflow-hidden rounded-lg w-40 h-40 flex-row items-center gap-3 mb-3">
                        <Image
